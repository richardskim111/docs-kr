...
참가자들은 웹사이트에서 스테이킹 양을 관리할 수 있습니다. 스테이킹 금액을 늘리면 NMR은 지갑에서 스테이킹 컨트랙트 (뉴머라이가 보유한 ETH 계정)로 전송됩니다. 스테이킹을 줄이면 NMR은 약 4주 지연 후에 참가자 지갑에 다시 이**지갑에 다시 이체됩니다**니다. 또한 스테이킹의 종류를 변경할 수도 있고 이를 통해 스테이킹 하고 싶은 스코어 (`corr`, `corr` + `mmc`)를 결정할 수 있습니다.
