...
{% hint style="success" %}
**참가자가 제출한 시그널이 높은 MMC를 나타낼 경우 다른 사람이 제출한 시그널보다 우위를 나타냅니다.**
{% endhint %}

**MMC는 뉴머라이 토너먼트에서 따온 개념으로 스코어링 시스템은 매우 유사합니다. 뉴머라이에서의 MMC 계산방법에 대한 자세한 내용은 뉴머라이 토너먼트 문서의 metamodel contribution 섹션을 참조해 주세요.**
